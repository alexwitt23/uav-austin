...
def eval(model: torch.nn.Module, loader: torch.utils.data.DataLoader) -> float:
    ...
    with torch.no_grad():
        ...
        for idx, (anchor, positive, negative) in enumerate(loader):
            ...
            pos_dist = metrics.pairwise_distances(out1.cpu(), out2.cpu())
            neg_dist = metrics.pairwise_distances(out1.cpu(), out3.cpu())
...
def create_data_loader(
    ...
    dataset = datasets.TargetDataset(
        data_dir, img_ext=generate_config.IMAGE_EXT, img_width=100, img_height=100,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Code used to train a feature extractor."
    )
    parser.add_argument(
        "--model_config",
        required=True,
